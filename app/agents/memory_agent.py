...
class MemoryAgent:
    ...
    async def analyze_conversation(self, conversation: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "You analyze resume conversations to learn about the user."},
    ...
    async def apply_learned_preferences(self, content: str) -> str:
        ...
        response = await self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "Rewrite text using the user's preferences and corrections."},
    ...
    async def learn_from_decision(self, decision: dict[str, Any]) -> dict[str, Any]:
        ...
        if new_skill:
            self.storage.remember_skill(new_skill, f"Review decision {decision.get('id', 'unknown')}")
