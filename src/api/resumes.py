...
class ResumeTailoringRequest(BaseModel):
    ...
    job_posting_text: str = Field(
        ...,
        min_length=50,
        description="Raw job posting text to tailor resume against"
    )
    use_stored_profile: bool = Field(
        True,
        description="Whether to use stored profile or require profile in request"
    )
    profile: UserProfile = Field(
        None,
        description="User profile data (required if use_stored_profile is False)"
    )
...
async def tailor_resume(request: ResumeTailoringRequest) -> ResumeTailoringResponse:
    ...
    try:
        ...
        await storage_service.save_session_data(
            ...
            pipeline_data=pipeline_results
        )
